...
import { PlanetData } from '../screens/PlanetsScreen/PlanetDetailsScreen';

// API para obtener la lista de planetas
export const getPlanets = async (order: string, page: string, search: string) => {
...
export const getPlanetDetails = async (id: string): Promise<PlanetData | null> => {
    const res = await axios({
      method: 'GET',
      url: API_URL + '/' + id,
    });
